					...
					foreach ($catalogo->result() as $producto)
					{
						if ($producto->tipo_producto == 1)
						{
							...
							<article class="article__producto">
								...
								<form class="form-compra" action="<?=base_url();?>AdministrarCliente/<?= $metodo?>/<?= $parametro?>" method="POST">
									<input type="number" name="producto_comprar" value="<?=$producto->id_producto?>" hidden>
									<input class="boton-compra" type="submit" value="Comprar">
								</form>
							</article>
					 <?php
					 	}
